...
import { Send, RefreshCw, Brain, Key } from 'lucide-react'
...
export default function UrduAssistant({ dark, lang, apiKey }) {
  const t = th(dark)
  const initialMsg = {
    id: '1',
    role: 'assistant',
  ...
  const sendMessage = async () => {
    ...
    try {
      ...
      const errText = err.message === 'NO_KEY'
        ? (lang === 'ur'
            ? 'API کی موجود نہیں۔ ترتیبات میں Claude API کی شامل کریں۔'
            : 'No API key found. Add your Claude API key in Settings → AI Preferences.')
        : (lang === 'ur' ? 'خرابی آئی۔ دوبارہ کوشش کریں۔' : `Error: ${err.message}. Please try again.`)
  ...
  return (
    <div style={{ padding: 24, height: 'calc(100vh - 72px)', display: 'flex', flexDirection: 'column' }}>
      ...
      {!apiKey && (
        <div style={{ marginBottom: 14, display: 'flex', alignItems: 'flex-start', gap: 10, padding: '12px 16px', borderRadius: 14, background: dark ? 'rgba(245,158,11,.08)' : '#fffbeb', border: `1px solid ${dark ? 'rgba(245,158,11,.2)' : '#fde68a'}` }}>
          <Key size={16} color='#f59e0b' style={{ flexShrink: 0, marginTop: 1 }} />
          <div style={{ fontSize: 11, color: dark ? '#fcd34d' : '#92400e' }}>
            {lang === 'ur'
              ? 'اے آئی چیٹ کے لیے ترتیبات میں Claude API کی شامل کریں۔'
              : 'Add your Claude API key in Settings → AI Preferences to enable AI chat.'}
          </div>
        </div>
      )}
