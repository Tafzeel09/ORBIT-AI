...
export default function SettingsPage({
  ...
  const saveApiKey = () => {
    ...
    lsSet('orbit_api_key', apiKeyDraft.trim())
    setApiKeySaved(true)
    setTimeout(() => setApiKeySaved(false), 2200)
  }
        ...
        return (
          <div>
            ...
            <p style={{ fontSize: 13, color: t.muted, marginBottom: 18 }}>
              {lang === 'ur' ? 'آپ کا ڈیٹا مکمل طور پر آپ کے ڈیوائس پر محفوظ ہے۔' : 'Your data stays 100% on your device. Nothing is sent to any server (except AI chat if API key is set).'}
            </p>
            {[
              ...
              { icon: '🤖', title: 'AI Chat',                desc: 'When using AI features, chat messages are sent to Anthropic API using your personal API key.' },
            ].map((item, i) => (
          ...
          <div>
            ...
            <div style={{ padding: '16px', borderRadius: 14, border: `1px solid ${t.border}`, background: t.hover, marginBottom: 18 }}>
              <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 8 }}>
                ...
                <span style={{ fontWeight: 700, fontSize: 13, color: t.text }}>Claude API Key</span>
              </div>
              <p style={{ fontSize: 11, color: t.muted, marginBottom: 10 }}>
                {lang === 'ur'
                  ? 'اے آئی فیچرز (چیٹ اور تجاویز) کے لیے Anthropic API کی درکار ہے۔ console.anthropic.com سے حاصل کریں۔'
                  : 'Required for AI Chat & Proposal Generator. Get yours free at console.anthropic.com'}
              </p>
              <div style={{ display: 'flex', gap: 8 }}>
                <input
                  ...
                  placeholder='sk-ant-...'
                  style={{ flex: 1, padding: '9px 12px', borderRadius: 10, border: `1.5px solid ${t.inputBdr}`, background: t.input, color: t.text, fontSize: 12, outline: 'none' }}
                />
                <Btn size='sm' onClick={saveApiKey} variant={apiKeySaved ? 'success' : 'primary'}>
                  <Save size={13} /> {apiKeySaved ? 'Saved!' : 'Save'}
                </Btn>
              </div>
              {apiKey && (
